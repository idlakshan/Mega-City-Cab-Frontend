...
const CustomerDashboardMain = () => {
  ...
  if (paymentHistoryData?.data) {
    const labels = Object.keys(paymentHistoryData.data);
    const values = Object.values(paymentHistoryData.data);
  ...
  if (isPaymentHistoryLoading || isUserLoading || isStatsLoading)
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-primary-black mb-8">
        Customer Dashboard
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
          ...
          <p className="text-2xl font-bold text-primary-black">
            LKR {userStats?.data?.totalSpending?.toFixed(2)}
          </p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
          <h4 className="text-sm text-gray-500 flex items-center">
          ...
          <p className="text-2xl font-bold text-primary-black">
            {new Date(userStats?.data?.activeSince).getFullYear()}
          </p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
          <h4 className="text-sm text-gray-500 flex items-center">
          ...
          <p className="text-2xl font-bold text-primary-black">
            {userStats?.data?.favoriteLocation}
          </p>
        </div>
      </div>
